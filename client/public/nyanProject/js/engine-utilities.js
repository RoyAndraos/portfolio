...
const addBackground = (root) => {
  ...
  bg.src = "/nyanProject/images/bg.jpg";
  ...
  liveCount.innerHTML = `❤️`;
  liveCount.style.position = "absolute";
  liveCount.style.top = "0";
  liveCount.style.margin = "10px";
  liveCount.setAttribute("id", "liveCount");
  liveCount.style.zIndex = "5";
  liveCount.style.fontSize = "15px";
  // We add it to the root DOM node
  root.append(bg);

  root.append(liveCount);
  ...
  start.style.top = "35px";
  start.style.left = "165px";
  start.style.fontSize = "16px";
  start.style.border = "none";
  start.style.background = "-webkit-linear-gradient(#f50198, #fefd63)";
  start.style.webkitBackgroundClip = "text";
  start.style.webkitTextFillColor = "transparent";
  start.style.color = "transparent";
  start.classList += "restart:hover";
  start.style.cursor = "pointer";
  start.style.fontWeight = "900";
  root.append(start);
  document.getElementById("app").appendChild(scoreBoard);

  const lifeLost = document.createElement("p");
  lifeLost.setAttribute("id", "youLostALife");
  root.append(lifeLost);
};
