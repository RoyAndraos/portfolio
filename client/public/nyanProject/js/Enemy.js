...
class Enemy {
  ...
  constructor(theRoot, enemySpot) {
    ...
    this.domElement.src = "/nyanProject/images/pineapple2.png";
  ...
  update(timeDiff) {
    ...
    if (this.y > GAME_HEIGHT) {
      ...
      if (SCORE % 25 === 0) {
        ...
        document.getElementById("level").innerHTML = `LEVEL ${levelNummber}`;
        let randomRoundNumber = Math.floor(Math.round() * 25);
        if (SCORE === randomRoundNumber) {
          //set src to random item in array
          //make said item do things aka add a life every 2 times player collects hearts
          // or shoot bullets at 'cats' (depending on bonus selected)
        }
      }
    }
  }
